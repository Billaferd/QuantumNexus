...
The paramount constraint is the 200 Requisition Point (RP) Budget. The main deck—which includes all Unit, Leader, Asset, Event, Upgrade, and Terrain cards—must contain a collection of cards whose total RP value equals exactly 200 or less. Crucially, there is no minimum or maximum limit on the physical number of cards in the main deck; the mathematical cost dictates the volume. Within this 200 RP limit, players are restricted to a maximum of 25 RP dedicated to Leader subtype cards, ensuring that powerful, game-altering personalities do not oversaturate the battlefield while maintaining isolation from the 15 CP live runtime economy.
