...
| Exhaust Target Card                                       | Changes a ready card to exhausted. (Significant tempo hit for opponent, or cost for self)                                                             | 3.5       |
| Ready Target Card (Deactivate)                            | Changes an exhausted card to ready. (Powerful tempo gain)                                                                                             | 4.0       |
| Apply **Suppressed** to Target Unit                       | Target Unit becomes exhausted (as per Rulebook Section III.C.2). (Equivalent to "Exhaust Target Card")                                                 | 2.5       |
| Remove **Suppressed** from Target Unit                    | If a Unit is Suppressed (and thus exhausted due to it), it becomes ready. (Equivalent to "Ready Target Card")                                        | 3.0       |
...
| Remove **Disordered** from Target Unit                    | Removes the -1 Defense penalty and Disordered state.                                                                                                  | 1.5       |
| Apply [Custom Defined State] to Target Card               | Applies a specifically defined temporary or ongoing state to a card. this cost is for the application of the state only. the state will need to be costed separately based on these appendices and added to this cost.         | 1.0 DB |
| Remove [Custom Defined State] from Target Card            | Removes a specifically defined state from a card. this is for the removal of the state only.                                                     | 2.5 DB |
...
| **Pushed Back** Target Unit                               | Controller moves the Unit one friendly cardinal quadrant (as per Rulebook Section III.C.2). (Part of combat; standalone effect might be costed)         | 1.5       |
...
| Grant Keyword [Keyword Name] to Target Card               | Target card gains a specified keyword for a duration. (Cost = DB cost of Keyword + small application fee)                                             | *Keyword DB + 1.0* |
| Remove Keyword [Keyword Name] from Target Card            | Target card loses a specified keyword. (Situational, moderate impact)                                                                                 | 2.0       |
| Target Card Gains [Ability Text]                          | Target card gains a specified rules text ability for a duration. This is for the application of the ability only, the ability will need to be costed and added according to these appendixes.                                         | 1.5 |
| Target Card Loses [Ability Text/All Abilities]            | Target card loses a specified ability or all its printed abilities for a duration. ("Silence" effect, powerful)                                       | 2.0 per ability, or 6.5 for "All" abilities |
...
| Destroy Target Card (Unit/Asset/Terrain/Upgrade)          | Target card is moved from its current zone to its owner's Discard Pile. (Unit: 4.0, Asset: 3.0, Terrain: 2.0, Upgrade: 1.0)                           | 1.0 - 4.0 |
...
| Place Card from Hand/Discard on Top/Bottom of [Player]'s Deck | Moves a card from one zone to a specific part of a deck. (Top of deck more impactful)                                                                 | 0.5 - 1.5 |
...
| Player Loses the Game                                     | Directly causes a player to lose (Core Principle: Immediate Loss applies). (High cost, likely conditional, or a major drawback on a powerful card)      | *Varies* |
| **E. Copying & Negating** |                                                                                                                                                         |            |
...
| **C. Cards in Deck** |                                                                                                                                                               |                             |
| Card(s) in Deck                                           | One or more cards within a specified player's Deck. Usually for searching, revealing, or direct manipulation. (For searching)                                     | +0.5 (if specific type)   |
...
| :-------------------------------------------------------- | :-------------------------------------------------------------------------------------------------------------------------------------------------------------- | :-------------------------- |
| **A. Battlefield Areas** |                                                                                                                                                               |                             |
| Quadrant                                                  | A single quadrant on any player's side of the Battlefield. Specify friendly/opponent, specific location if needed.                                              | 0.0                         |
...
| At the End of the Activation Sequence                     | Triggers when all players have consecutively passed, ending the Activation Sequence for the Game Round. (Reliable, once per round)                                                                                                | 1.5       |
...
| When [This/Target/Any] Unit is Destroyed (via Attack/Impact) | Triggers after combat/Impact resolution if the specified Unit was destroyed. ("This Unit": 1.0)                                                                                                                                   | 1.0 - 2.0 |
...
| If [Player]'s Discard Pile contains [Number/Type/Criteria of] Card(s)          | E.g., "If there are 5 or more Unit cards in your Discard Pile," "If your Discard Pile is empty." (Enables graveyard strategies, can be build-around)         | -0.25 to -0.75     |
