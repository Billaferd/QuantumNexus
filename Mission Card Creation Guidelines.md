...
Mission cards are used to define the number of points in each domain. The domain points across all Mission cards must add up to six (6). Each domain points gives 3 CP minimum. The total CP across all domain cards is the amount of CP the player may spend per round. Each Mission Card must have one Challenge. Once the challenge is completed the player may trigger a one-time ability and also a passive bonus for each turn after the challenge has been completed. The challenges will grow in difficulty on a logarithmic scale as total number of points increase on the Mission card. Each Mission card may also have a One-time ability and a passive ability. these abilities have specific costs that can't exceed the number of CP that the mission card has generated. If the One-Time and Passive abilities scores are lower than the number of CP points generated by the Mission Card, the difference is added to the total CP of the card.

'''
Target CP = Domain Points * 3
Total CP = Target CP + (Target CP - (One-Time Effect Cost + Passive Effect Cost))
'''

Once the mission cards are selected the player will not be able to select any cards with domain values higher than the sum of the respective domain values on the mission cards. Any one mission card can't have any more than four (4) domain points.
...
- Mission cards must allocate between 1 and 4 points across the domains: Technology, Magic, Psionics, and Divinity.
- No individual domain should exceed a maximum of 4 points on a single mission card.
- The total domain points on a card can range from 1 to 4, but individual domains must range between 0 and 4 points. A mission card can assign points to multiple domains.
...
2. **Domain Points to CP Conversion**: To determine how many CP each domain point contributes to a mission card, follow these guidelines:
   - Each domain point allocated to a mission card contributes 3 CP to its CP cost.
   - The total CP cost of a mission card is calculated by summing the CP contributions from all its allocated domain points.

3. **Individual Domain Limit**: No single domain point on a mission card can contribute more than 3 CP to the card's total CP cost. This ensures the CP distribution remains balanced and prevents excessive CP generation from a single domain.

4. **Minimum and Maximum CP**: A mission card's CP cost must fall within 1 to 12 CP. This range maintains balance and prevents mission cards from being overpowered or weak.

5. **Challenging CP Allocations**: To create challenging mission cards, allocate domain points so that the CP cost is proportional to the challenge's difficulty. Higher-difficulty challenges should generally have higher CP costs, reflecting the complexity and significance of the task.
...
One domain point should be an easy challenge. 2 points would be an intermediate challenge. 3-4 points should be an advanced challenge. The points come from the sum of all domain points used on the card. If a card has 1 point in Technology and 2 points in Psionics, that would be 3 points for the challenge.
...
- 4 Points - Tap into the divine mysteries of the mind. Deploy a unit with psionics and divinity attributes and command it to initiate a spiritual awakening that affects all units on the battlefield.

#### Technology, Magic, and Divinity Challenge

- 4 Points - Combine technology, magic, and divine energies to create a transcendent artifact. Deploy a legendary unit with all three attributes and trigger its ultimate ability to reshape the very fabric of reality.
