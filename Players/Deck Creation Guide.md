...
Crucially, the rules place a strict limit of 40 to 60 physical cards within the main deck, existing simultaneously alongside the 200 RP limit. This creates a fundamental departure from games that rely on a single constraint. Quantum Nexus instead employs a dual-layered "point-buy" system, forcing a critical strategic tension between the quantity of cards, mathematical consistency, and their individual quality, as measured by RP cost.

This system presents the architect with a profound choice. One could construct a streamlined 40-card deck to maximize draw consistency, ensuring they find their key combo pieces quickly. However, this hyper-consistency leaves the architect with an average Requisition Point allowance of exactly 5 RP per card, potentially starving them of high-impact late-game options. Conversely, one could build a 60-card deck to mathematically dilute their RP average, allowing them to legally afford a massive suite of elite, expensive powerhouses and Leaders without exceeding the 200 RP cap. This decision between a highly consistent but individually weaker arsenal and a larger, slightly more varied deck packed with heavy artillery is a core component of a deck's identity, influencing everything from opening hands to late-game consistency.
...
Main Deck Size,40 to 60 Cards,"Unit, Leader, Asset, Event, Upgrade, Terrain"
...
The Mission system is built on a clever set of limitations that force strategic trade-offs. A player must include up to a maximum of six Mission cards, and their chosen package must adhere to two strict rules: the total of all Domain Points across all Mission cards must equal exactly 6, and no single domain (Technology, Magic, Psionics, or Divinity) can have more than 4 points.
...
A 4-1-1-0 spread creates a highly focused deck in a primary domain, with two minor "splashes" into other domains for utility. This is ideal for strategies that rely on a powerful "Keystone" mission, such as a Technology deck built around the 4 DP Singularity Engine.

A 2-2-2-0 spread results in a balanced, three-domain deck, granting access to a wider variety of tools but without the game-defining power of a 4 DP mission.

A 3-2-1-0 spread creates a "major/minor/splash" identity, offering a primary strategic direction with significant support from a secondary domain and a touch of utility from a third.
...
Logistics (Theater 1): The standardized 15 CP salary funds the early war (0-5 Combined Points).

Escalation (Theater 2): A passive doctrine becomes available for activation or sustained upkeep via a dynamic runtime CP cost when the tension rises (6-9 Combined Points).

The Climax (Theater 3): A massive, one-time reward can be manually triggered by paying its steep runtime CP cost when total war is achieved (10+ Combined Points).
...
If an architect selects this Mission, they are actively choreographing a late-game combo. The deck must include efficient artillery units (Range 3+) to exploit the Theater 2 passive, but the true goal is surviving until Theater 3. The moment the global board hits 10 combined Supremacy Points, this Mission can be manually detonated for 5 CP, crippling the opponent's board. Deck building is no longer about fulfilling micro-objectives; it is about managing the chronological pacing of the war.
...
This dynamic is profoundly amplified by the game's Momentum rule and strict Initiative alternation. Because the Initiative token strictly passes back and forth between players at the start of every new round, tempo is highly predictable, allowing players to accurately forecast the turn order and set up two-turn tactical maneuvers. Furthermore, the 'Momentum' rule—where completely clearing a target quadrant of all enemy units allows the attacker to immediately advance into that space for free—means that combat results in immediate territorial gains. A player might intentionally deploy a 1-Weight sacrificial unit to stall an opposing Massive unit's advance, forcing the opponent to spend an action clearing it. Mastering this delicate balance—knowing how to manipulate the 15 CP economy to engineer a fluid frontline sweep or deliberately leveraging a Large unit's native 'Pushback' trait to rip holes in the enemy line—is a hallmark of an expert Nexus Architect.
...
Tempo, on the other hand, is the pace of play. It is the measure of how effectively a player uses their Command Points and actions within the game's alternating Activation Sequence to advance their board state relative to their opponent's. A critical and unique aspect of tempo in Quantum Nexus is the management of the "Fluid Frontline" via the Momentum rule and strict alternating Initiative. Because Initiative simply passes to the opposing player at the end of every round, you can precisely calculate who will act first in the future. Tempo swings are intensely violent due to the Momentum rule—if your attack clears an enemy quadrant, you immediately occupy it. This creates a direct and powerful link between spatial control and action economy. A player might deliberately use a Large unit's innate 'Pushback' trait to shove an enemy out of position, or execute a mathematically precise area-of-effect strike with a Massive unit's 'Quadrant Damage' to clear three Small tokens and surge forward. Because enemy quadrants are strictly impassable walls governed by a 3-Weight capacity limit, weaponizing aggression requires flawless spatial awareness. If the round devolves into a stalemate, the player who physically occupies the most quadrants in the neutral Contested Zone (Row 3) secures Supremacy Points, accelerating the Theater Escalation and punishing passive play. The highest level of strategic skill in Quantum Nexus is knowing exactly when to trade CP for a Momentum-driven territorial advance, and when to spend 3 CP on the 'Draw Intel' action to reload your hand while setting up an impenetrable defensive line.
